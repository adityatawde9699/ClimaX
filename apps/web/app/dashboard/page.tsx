...
function IncidentQueue({ items }: { items: Incident[] }) {
  ...
  const [sort, setSort] = useState<'severity' | 'status' | 'assignee' | 'created_at'>('severity');
  ...
  const sorted = [...items].filter((item) => !['RESOLVED', 'DISMISSED'].includes(item.status)).sort((left, right) => sort === 'severity' ? (severityRank[right.severity] ?? 0) - (severityRank[left.severity] ?? 0) : sort === 'created_at' ? Date.parse(right.updated_at) - Date.parse(left.updated_at) : sort === 'assignee' ? (left.assigned_officer_id ?? 'Unassigned').localeCompare(right.assigned_officer_id ?? 'Unassigned') : left.status.localeCompare(right.status));
